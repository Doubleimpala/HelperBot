...
module.exports={
  ...
  async excecute(message, args, client){
    ...
    if(args[0]) {
      ...
      if(!ctg) return message.reply('Invalid Command. This bot doesn\'t have that command.'), message.react('❌')
      ...
      let cmds = client.commands.map(command => command.name)
       ...
       message.author.send(allEmbed)
       message.channel.send(allEmbed)
       message.channel.send(`**I also sent the same in your DMs as it may get lost in the channel.**`)
